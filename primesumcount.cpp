...
Go to http://www.icecreambreakfast.com/math/PrimeSumming_NathanMcKenzie.pdf for a more detailed description of what's going on in
this algorithm.

This file contains an algorithm for computing the sum of the primes to the power of some non-negative integer power SUM_POWER up to
some cut off number n.  It should run in the ballpark of O(n^2/3 log n) time and O(n^1/3 log n) space.  

Change the define SUM_POWER to raise the prime to different powers.  Currently only int values of 0 through 3 will have meaningful timings.

As currently written, static BigInt countprimes(BigInt num, double basePow ) is the function that runs the algorithm.  This file's main
function currently runs and times the algoirthms for subsequent powers of 10 .
*/
#define SUM_POWER 0
/*
Note that, as it stands, it's enough to demonstrate the general algorithm and its timing, but not enough to actually be used - there are already
enough lurking precision issues (particularly with the pow function) that the algorithm is often off by a few for even not particularly
large values.  Additionally, for any SUM_POWER > 0, even 64 bit ints run out of precision pretty quickly here, so that would likely need to be
replaced.

I also really haven't done any meaningful constant time performance optimizations.

It currently only works for 0 <= SUM_POWER <= 3 as implemented - the function addRange implements the first few values of Faulhaber's formula,
http://mathworld.wolfram.com/FaulhabersFormula.html .  Add more entries to support higher powers.

It's possible that this algorithm could support negative or non-integer powers if there are constant or polynomials time ways to replace the
...
If this code were to be used to compute negative or non-integer powers, the code would have to be scoured for precision issues, as it currently
assumes long integers for most computation.

NOTE!  This was originally written as a prime counting algorithm - the current changes represent a generalization of that algorithm.  Thus, the
comments often refer to the original prime counting approach - there are some subtle difference between the two to support the generalization,
...
/* 
This function adds entries from start+1 through finish (so, finish - start, essentially).
This only works if there is some polynomial time way of computing the sum
(start+1)^theCurPow + (start+2)^theCurPow + ...+ (finish-1)^theCurPow + finish^theCurPow
*/
static BigInt addRange( BigInt start, BigInt finish ){
	BigInt cnt = 0;
	if( theCurPow == 0 ){  cnt = finish - start; }
	else if( theCurPow == 1 ) cnt = finish*(finish + 1 )/2 - start*(start + 1 )/2;
	else if( theCurPow == 2 ) cnt = finish*(finish + 1 )*(2*finish+1)/6 - (start)*(start + 1 )*(2*start+1)/6;
	else if( theCurPow == 3 ) cnt = finish*finish*(finish+1)*(finish + 1 )/4 - start*start*(start+1)*(start+1)/4;
	else for( BigInt rr = start+1; rr <= finish; rr++ )cnt += pow( rr, theCurPow );
	return cnt;
}
...
static void allocPools( BigInt n ){
    ...
    logn = (BigInt)((log(pow(n, 2.0000000 / 3.0)+.0000000001)+.00000001) / log(2.0)) + 1;
    ...
    numDPowers = (BigInt)(.00000001 + (log(pow(n, 2.0000000 / 3.0) + .00000001)+.00000001) / log(2.0)) + 1;
...
static void factorRange(){
    for (BigInt j = 0; j < numPrimes; j++){
        ...
        BigInt cap = (BigInt)( log((double)(nToTheThird+curBlockBase)) / log((double)curPrime) + 1 + .000000001);
...
static void buildDivisorSums(){
    for (BigInt j = 1; j < nToTheThird+1; j++){
        ...
        for (BigInt k = 0; k < logn; k++){
            DPrime[j * logn + k] = DPrime[(j - 1) * logn + k] + (BigInt)( d2(factors, j - 1, k, numPrimeBases[j - 1] + 1, totalFactors[j - 1])*pow( curBlockBase+j-1, theCurPow) );
        }
    }
...
static void calcS1(){
    ...
    if (S1Mode == 1){
        while (n / S1Val <= ceilval){
            ...
            for (BigInt m = 1; m < numDPowers; m++) t += cnt * (m % 2 == 1 ? -1 : 1) * (1.0 / (m + 1)) * DPrime[(n / S1Val - curBlockBase + 1) * logn + m];
...
static void calcS2(){
    for (BigInt j = 2; j <= nToTheThird; j++)
        for (BigInt k = 1; k < numDPowers; k++){
			...
			t += thecnt * pow(-1.0, (double)k) * (1.0 / (k + 1)) * (DPrime[j * logn + k] - DPrime[(j - 1) * logn + k]);
		}
}
    ...
    for (BigInt j = 2; j <= nToTheThird; j++){
        ...
        if (S3Modes[j] == 1){
            while (n / (j * S3Vals[j]) <= ceilval){
                ...
                for (BigInt m = 1; m < numDPowers; m++) t += DPrime[(n / (j * S3Vals[j]) - curBlockBase + 1) * logn + m] * dPrime[j * (numDPowers + 1) + m] * cnt;
...
static BigInt countprimes(BigInt num, double basePow ) {
    ...
    for (BigInt i = 1; i <= (log((double)num)+.000000001) / log(2.0); i++) {
		theCurPow = basePow * i;
		double val = calcPrimePowerCount( pow( (double)num, 1.0/i ) + .0000000001, basePow * i) / (double)i * mu[i];
		total += val;
    }
